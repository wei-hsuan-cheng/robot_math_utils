...
public:
    ...
    // Adjoint representations on Lie group (velocity/twist adjoint maps)
    ...
    // Adjoint representations on Lie algebra (Lie bracket; 6D twist cross product)
    static inline Matrix6d adjoint(const Vector6d& se3Vec) {
        const Vector3d v = se3Vec.head<3>();
        const Vector3d wv = se3Vec.tail<3>();
        Matrix6d ad; ad.setZero();
        ad.topLeftCorner<3,3>()  = R3Vec2so3Mat(wv);
        ad.topRightCorner<3,3>() = R3Vec2so3Mat(v);
            ad.bottomRightCorner<3,3>() = R3Vec2so3Mat(wv);
            return ad;
    }
