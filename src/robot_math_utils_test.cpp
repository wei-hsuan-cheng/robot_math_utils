...
int main(int argc, char** argv) {
    ...
    // Test RandNorDistVec for generating N = 5000 data points
    std::cout << "\n----- Test RandNorDistVec (Generating 5000 data points) -----\n" << std::endl;
    int N_rand = 5000;  // Number of data points
    VectorXd mean(2), std_dev(2);
    mean << 1.0, 5.0;
    std_dev << 0.5, 0.1;
    MatrixXd cov = std_dev.cwiseProduct(std_dev).asDiagonal();
    Eigen::MatrixXd rand_vecs(N_rand, mean.size());
    for (int i = 0; i < N_rand; ++i) {
        VectorXd rand_vec = RM::RandNorDistVec(mean, cov);
        rand_vecs.row(i) = rand_vec.transpose();
    }
    std::string filename = datalog_path + "/rand_vecs_" + std::to_string(N_rand) + ".csv";
    RM::SaveMat(rand_vecs, filename);
    std::cout << "Random vectors saved to " << filename << std::endl;

    // Test quat with z-axis pi rotation
    std::cout << "\n----- Test quat with z-axis pi rotation -----\n" << std::endl;
    Quaterniond quat = RM::Quatz(M_PI);
    ...
    quat = RM::zyxEuler2Quat(zyx_euler * RM::d2r);
    ...
    std::cout << "\n----- Test inverse quaternion -----\n" << std::endl;
    zyx_euler = Vector3d(30, 15, 45);
    quat = RM::zyxEuler2Quat(zyx_euler * RM::d2r);
    Quaterniond quat_conj = RM::ConjQuat(quat);
    Quaterniond quat_inv = RM::InvQuat(quat);
    Quaterniond quat_quat_conj = quat * quat_conj;
    Quaterniond quat_quat_inv = quat * quat_inv;
    std::cout << "quat = [" << quat.w() << ", " << quat.x() << ", " << quat.y() << ", " << quat.z() << "]" << std::endl;
    std::cout << "quat_conj = [" << quat_conj.w() << ", " << quat_conj.x() << ", " << quat_conj.y() << ", " << quat_conj.z() << "]" << std::endl;
